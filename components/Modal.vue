<template>
  <div class="modal-bg" @click.self="$emit('close')" v-if="window_width >= 600">
    <div class="modal-item">
      ...
      <div class="modal-detail">
        ...
        <p>
          使用言語<br /><span>{{ val.m_pic_skill }}</span>
        </p>
        <p>
          サイトURL<br /><span>{{ val.m_site_url }}</span>
        </p>
      </div>
    </div>
  </div>
...
export default Vue.extend({
  ...
  props: {
    // 受け取る属性名を指定
    val: Object as Vue.PropType<Item>, // オブジェクトのみ受け取る
  },
  data() {
    return {
